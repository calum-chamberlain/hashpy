...
if not interactive:
    ...
    dang = input(
        "Enter grid angle for focal mech search, in degrees (min {0}): ".format(dang0)
    )
...
while True:
    ...
    while True:
        ...
        spol = check_pol(
            plfile, sname[k], iyr, imon, idy, ihr
    ...
    if str_avg[0] == 999:
        ...
        nmult, str_avg, dip_avg, rak_avg, prob, var_est = mech_prob(
            f1norm[:, :nout2], f2norm[:, :nout2], cangle, prob_max, nout2
        )  # nout2
        ...
        for imult in range(nmult):
            ...
            if (
                (prob[imult] > 0.8)
                and (var_avg[imult] < 25)
                and (mfrac[imult] <= 0.15)
                and (stdr[imult] >= 0.5)
            ):
                qual[imult] = "A"
            elif (
                (prob[imult] > 0.6)
                and (var_avg[imult] <= 35)
                and (mfrac[imult] <= 0.2)
                and (stdr[imult] >= 0.4)
            ):
                qual[imult] = "B"
    ...
    for i in range(nmult):
        ...
        fh13.write(
            format13.format(
                ...
                int(round(dip_avg[i])),
                int(round(rak_avg[i])),
                int(round(var_est[0, i])),
                int(round(var_est[1, i])),
                npol,
                int(round(mfrac[i] * 100.0)),
                qual[i],
                int(round(100 * prob[i])),
                int(round(100 * stdr[i])),
                mflag,
            )
        )
    ...
    if go_again == '' or go_again in 'Yesyes':
        ...
        test_stereo(
            ...
            p_the_mc[:npol, 0],
            p_pol[:npol],
            sdr=[str_avg[0], dip_avg[0], rak_avg[0]],
        )
        fh11.close()
        fh12.close()
        fh13.close()
