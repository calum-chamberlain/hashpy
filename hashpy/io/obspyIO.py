...
def inputOBSPY(hp, event):
    ...
    try:
        _o = event.preferred_origin() or event.origins[-1]
    except IndexError as e:
        raise NotImplementedError("Requires an origin")

    try:
        _m = event.preferred_magnitude() or event.magnitudes[-1]
    except IndexError as e:
        _m = None
...
def outputOBSPY(hp, event=None, only_fm_picks=False):
    ...
    if event is None:
        ...
        for _i in range(n):
            ...
            p.resource_id = ResourceIdentifier(
                "smi:hash/Pick/{0}".format(p.creation_info.version)
            )
            ...
            a.resource_id = ResourceIdentifier(
                "smi:hash/Arrival/{0}".format(p.creation_info.version)
            )
    ...
    for s in range(hp.nmult):
        ...
        focal_mech.creation_info = CreationInfo(
            creation_time=UTCDateTime(), author=hp.author
        )
        ...
        focal_mech.principal_axes.t_axis = Axis(
            azimuth=ax["T"]["azimuth"], plunge=ax["T"]["dip"]
        )
        focal_mech.principal_axes.p_axis = Axis(
            azimuth=ax["P"]["azimuth"], plunge=ax["P"]["dip"]
        )
