...
class FocalMechPlotter(object):
    ...
    def plot_on_stereonet(self, axis=None, fm=None):
        ...
        for ind, a in enumerate(self._orig.arrivals):
            ...
            if 0.0 <= a.takeoff_angle < 90.0:
                toa = 90.0 - a.takeoff_angle  # complement for downward angles
            elif 90.0 <= a.takeoff_angle <= 180.0:
                toa = 270.0 - a.takeoff_angle  # project upward angles
            else:
                raise ValueError(
            ...
            if True:
                h_text = ax.rake(
                    ...
                    toa + 5,
                    90,
                    marker="$   {0}$".format(p.waveform_id.station_code),
                    color="black",
                    markersize=20,
                )
        ...
        h_text = self.fig.text(
            0.25,
            0.88,
            plane_str.format(strike1, dip1, rake1, strike2, dip2, rake2),
            ha="right",
            va="top",
            family="monospace",
        )
        h_text = self.fig.text(
            0.25,
            0.11,
            "Quality:  {0}\n# of picks: {1}".format(qual, len(self._orig.arrivals)),
            ha="right",
            va="top",
            family="monospace",
        )
    ...
    def _set_window_title(self):
        self.fig.canvas.set_window_title(
            "Focal Mechanism: {0} of {1}".format(self._fm_index + 1, self._num_fms)
        )
